...
RSpec.describe Provider, type: :model do
  ...
  describe "validations" do
    ...
    it { is_expected.not_to allow_value("1234").for(:ukprn).with_message("Enter UK provider reference number (UKPRN)") }
    ...
    it { is_expected.not_to allow_value("abcdx").for(:code).with_message("Enter provider code") }

    context "when provider_type is school or scitt" do
      [:school, :scitt].each do |provider_type|
        let(:provider) { build(:provider, provider_type, urn: nil) }
