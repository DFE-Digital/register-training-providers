...
RSpec.describe ApiClient, type: :model do
  ...
  describe "before_discard callback" do
    let!(:active_token1) { create(:authentication_token, expires_at: 1.day.from_now, api_client: api_client) }
    let!(:active_token2) { create(:authentication_token, expires_at: 1.day.from_now, api_client: api_client) }
    let!(:expired_token) { create(:authentication_token, :expired, expires_at: Date.current, api_client: api_client) }
  ...
  describe "#expire_all_due_tokens!" do
    let!(:due_token1) { create(:authentication_token, api_client: api_client, expires_at: Date.current) }
    let!(:due_token2) { create(:authentication_token, api_client: api_client, expires_at: 1.day.from_now) }
  ...
  describe ".sweep_all_tokens!" do
    ...
    let!(:due_token1) { create(:authentication_token, api_client: kept_client, expires_at: Date.current) }
    let!(:due_token2) { create(:authentication_token, api_client: kept_client, expires_at: 1.day.from_now) }
