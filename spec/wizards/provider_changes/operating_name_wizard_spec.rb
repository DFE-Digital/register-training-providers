RSpec.describe ProviderChanges::OperatingNameWizard do
  ...
  let(:state_store) do
    ProviderChanges::StateStores::OperatingNameStore.new(repository:)
  end
  ...
  describe "step graph" do
    ...
    it do
      expect(wizard)
        .to have_next_step(:new_operating_name)
        .from(:effective_date)
    end
  ...
  describe "routes" do
    ...
    it "routes new_operating_name" do
      expect(wizard).to resolve_step(:new_operating_name).to(
        provider_change_update_step_path(
          provider_id: provider.id,
          field: "operating_name",
          step: "new-operating-name"
        )
      )
    end
  ...
  describe "review navigation" do
    ...
    context "when returning to review" do
      let(:current_step_params) { { return_to_review: "new_operating_name" } }
    ...
    context "when returning from review" do
      let(:current_step) { :new_operating_name }
      let(:current_step_params) { { return_to_review: "new_operating_name" } }
  ...
  describe "check_your_answers?" do
    ...
    context "when current_step is new_operating_name" do
      let(:current_step) { :new_operating_name }
  ...
  describe "#set_state_store" do
    ...
    context "when no provider change is present" do
      it "does not change the state store" do
        state_store.write(
          operating_name: "Another provider name",
