RSpec.describe ProviderChanges::UkprnWizard do
  ...
  let(:state_store) do
    ProviderChanges::StateStores::UkprnStore.new(repository:)
  end
  ...
  describe "routes" do
    ...
    it "routes new_ukprn" do
      expect(wizard).to resolve_step(:new_ukprn).to(
        provider_change_update_step_path(
          provider_id: provider.id,
          field: "ukprn",
          step: "new-ukprn"
  ...
  describe "review navigation" do
    ...
    context "when returning to review" do
      let(:current_step_params) { { return_to_review: "new_ukprn" } }
  ...
  describe "check_your_answers?" do
    ...
    context "when current_step is new_ukprn" do
      let(:current_step) { :new_ukprn }
