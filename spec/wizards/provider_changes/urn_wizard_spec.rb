RSpec.describe ProviderChanges::UrnWizard do
  ...
  let(:state_store) do
    ProviderChanges::StateStores::UrnStore.new(repository:)
  end
  ...
  describe "routes" do
    ...
    it "routes new_urn" do
      expect(wizard).to resolve_step(:new_urn).to(
        provider_change_update_step_path(
          provider_id: provider.id,
          field: "urn",
          step: "new-urn"
  ...
  describe "review navigation" do
    ...
    context "when returning to review" do
      let(:current_step_params) { { return_to_review: "new_urn" } }
  ...
  describe "check_your_answers?" do
    ...
    context "when current_step is new_urn" do
      let(:current_step) { :new_urn }
