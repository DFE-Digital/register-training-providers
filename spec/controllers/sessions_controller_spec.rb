...
RSpec.describe SessionsController, type: :controller do
  ...
  describe "#callback" do
    context "existing database user" do
      ...
      it "redirects to the provider index page if no original path stored in the session" do
        request_callback
        expect(response).to redirect_to(providers_path)
      end

      it "redirects to the original requested page if it exists in the session" do
        session[:requested_path] = "/original/requested/page"
        request_callback
        expect(response).to redirect_to("/original/requested/page")
      end

      it "clears the redirect_back_to key of the session after a redirect" do
        session[:requested_path] = "/this-should-be-nil"
        request_callback
